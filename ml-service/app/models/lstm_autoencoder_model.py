...
class LSTMAutoencoderWrapper:
    def __init__(self):
        ...
        if self.torch_enabled:
            ...
            print("[LSTMAutoencoderWrapper] PyTorch not available, using Lightweight Statistical Fallback.")

    def load_or_init(self):
        if self.torch_enabled and os.path.exists(settings.LSTM_MODEL_PATH):
            try:
    ...
    def predict_sequence(self, sequence: list) -> dict:
        """
        Sequence expected: list of dicts with keys ['temperature', 'humidity', 'pressure']
        """
        ...
        if self.torch_enabled:
            try:
                ...
                print(f"[LSTMAutoencoderWrapper] Inference warning: {e}, falling back to statistical metric")

        # Statistical fallback evaluation
        variance = float(np.mean(np.var(norm_arr, axis=0)))
        anomaly_score = float(np.clip(variance * 0.5, 0.0, 1.0))
        return {
            "reconstruction_error": round(variance, 5),
            "anomaly_score": round(anomaly_score, 4),
            "is_anomaly": anomaly_score > 0.45
        }
